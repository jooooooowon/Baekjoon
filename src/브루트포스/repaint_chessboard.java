...
public class repaint_chessboard {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i <= y - 8; i++) {
			for(int j = 0 ; j <= x - 8; j++) {
				int num = repaint_chessboard(arr,j,i);		
				int num2 = re_repaint_chessboard(arr, j, i);
				result = Math.min(result,Math.min(Math.min(num, 64 - num),Math.min(num2, 64 - num2)));
			}
		}
		int min = 64;
	...
	private static int re_repaint_chessboard(String[] arr, int x, int y) {
		int count = 0;
		if(arr[y].charAt(x) == 'B') {
			if((x + y) % 2 == 1) {
				for(int i = y; i <= y + 7; i++) {
